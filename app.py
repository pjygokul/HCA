...
from ai_model       import train_all_models, generate_forecasts, get_feature_importance_df, generate_risk_clusters
...
if "chat_history" not in st.session_state:
    st.session_state.chat_history = [{"role": "assistant", "content": "Hello! I am the Chennai Flu Watch AI assistant. How can I help you today? (e.g. 'Which area has highest flu cases?')", "highlight_zone": None}]
if "highlighted_zone" not in st.session_state:
    st.session_state.highlighted_zone = None
...
def run_pipeline(days: int = 180, horizon: int = 14, total_icu: int = 150, total_docs: int = 300, total_o2: int = 800):
    ...
    # Apply Unsupervised ML Clustering (AI Spatial Technique)
    urgency_df  = generate_risk_clusters(urgency_df, n_clusters=3)
...
with st.sidebar:
    ...
    map_view = st.radio("Map Overlay Style", ["Urgency Plot", "AI Risk Clusters"], index=0)
...
with col_map:
    deck = build_map(urgency_df, map_view_mode=map_view, highlighted_zone=st.session_state.highlighted_zone)
    st.pydeck_chart(deck, use_container_width=True)
    if st.session_state.highlighted_zone:
        st.warning(f"📍 Map is currently locked on **{st.session_state.highlighted_zone}** based on chat.", icon="🔍")
    elif map_view == "Urgency Plot":
        st.caption("🔴 High · 🟡 Medium · 🟢 Low  |  Circle size = urgency score")
    else:
        st.caption("🤖 Unsupervised Learning Risk Profiles via K-Means Clustering")
...
table_df = alloc_df[[
    "zone", "priority", "risk_profile", "urgency_score",
...
table_df.columns = [
    "Zone", "Priority", "Risk Profile", "Urgency", "Pred Cases (7d)",
...
# ── Row 6: Healthcare Chatbot Assistant ───────────────────────────────────────
st.markdown('<div class="section-header">🤖 Healthcare AI Assistant</div>', unsafe_allow_html=True)
...
# Chat Input
if prompt := st.chat_input("Ask about flu hotspots, fastest growing areas, or ICU needs..."):
    # Add user message
    st.session_state.chat_history.append({"role": "user", "content": prompt, "highlight_zone": None})
    with chat_container:
        with st.chat_message("user"):
            st.markdown(prompt)
    
    # Process query
    bot_reply = process_chat_query(prompt, alloc_df)
    ans_text = bot_reply["text"]
    hl_zone = bot_reply["highlight_zone"]
    
    # Update state
    st.session_state.chat_history.append({"role": "assistant", "content": ans_text, "highlight_zone": hl_zone})
    if hl_zone:
        st.session_state.highlighted_zone = hl_zone
    
    # Render response and inherently trigger re-run for map
    st.rerun()